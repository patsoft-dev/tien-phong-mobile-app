  ...
  Widget build(BuildContext context) {
    ...
    for (int i = 0; i < 5; i++) {
      if (i < ratingCount) {
        ...
        if (isHalf) {
          isHalf = false;
          stars.add(Icon(halfIcon ?? Icons.star_half_outlined, color: color, size: size));
        } else if (showInactive) {
          stars.add(
            Icon(inactiveStarFilled ? (activeIcon ?? Icons.star) : (inactiveIcon ?? Icons.star_outline), color: inactiveColor, size: size),
          );
        }
        stars.add(SizedBox(width: spacing));
      }
